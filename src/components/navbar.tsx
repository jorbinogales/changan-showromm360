...
const NavbarC = () => (
	<nav className="p-4 font-play text-primary">
		<div className="max-w-[1300px] w-full mx-auto flex justify-between items-center auto">
			<svg
			...
			<ul className="flex space-x-6">
				<li>
					<p className="cursor-pointer hover:underline text-primary-changan play-regular">
						Inicio
					</p>
				</li>

				<li className="relative group cursor-pointer">
					<p className="flex items-center text-primary-changan play-regular">
						Descubre Changan <ChevronDown />
					</p>
					<ul className="absolute hidden group-hover:block bg-white shadow-lg rounded mt-2 py-2 w-56 text-sm">
						{[
							'Changan Global',
							'Changan en Venezuela',
							'Repuestos y Servicios',
							'Aporte Social (R.S.E)',
							'Changan y la Vinotinto',
							'Blog',
						].map((item) => (
							<li
								key={item}
								className="px-4 py-2 hover:bg-gray-100 text-primary font-play"
							>
								{item}
							</li>
						))}
				...
				<li className="relative group cursor-pointer">
					<p className="flex items-center text-primary-changan play-regular">
						Vehículos <ChevronDown />
					</p>
					<ul className="absolute hidden group-hover:flex flex-wrap w-[600px] bg-white shadow-lg mt-2 p-4 rounded text-sm gap-4">
						{[
							{ name: 'Pick Up Hunter', desc: '1000 KG vs 4x4 y 4x2' },
							{ name: 'SUV CS35 PLUS', desc: '1.4L Turbo, Normal y Sport' },
							{ name: 'SUV CS55 PLUS', desc: '1.5L Turbo, Económico y Sport' },
							{ name: 'SUV CS15', desc: 'Motor 1.5L Gasolina, 4 cilindros' },
							{
								name: 'Eléctrico Benni',
								desc: '100% Eléctrico, 300km Autonomía',
							},
							{ name: 'Sedán Alsvin', desc: 'Blue Core Motor' },
						].map((v) => (
							<li
								key={v.name}
								className="w-[180px] px-2 py-2 hover:bg-gray-100 text-primary font-play"
							>
				...
				<li className="relative group cursor-pointer">
					<p className="flex items-center text-primary-changan play-regular">
						Contacto <ChevronDown />
					</p>
					<ul className="absolute hidden group-hover:block bg-white shadow-lg rounded mt-2 py-2 w-48 text-sm">
						<li className="px-4 py-2 hover:bg-gray-100 text-primary font-play">
							Agenda tu cita
						</li>
					</ul>
				</li>
